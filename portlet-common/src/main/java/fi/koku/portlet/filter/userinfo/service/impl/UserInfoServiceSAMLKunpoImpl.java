...
public class UserInfoServiceSAMLKunpoImpl implements UserInfoService {
  ...
  private static final Logger LOG = LoggerFactory
      .getLogger(UserInfoServiceSAMLKunpoImpl.class);
  ...
  public UserInfo getUserInfoById(String pic) {
    ...
    if (StringUtils.isNotBlank(pic)) {
      ...
      try {
        List<Person> persons = ps.getPersonsByPics(user,
            PersonConstants.PERSON_SERVICE_DOMAIN_CUSTOMER, pic,
            UserInfoConstants.COMPONENT_USER_INFO_FILTER);
        Person p = persons.get(0);// Get the first one (actually should there
                                  // should be only 0 or 1 persons in list)

        if (p != null && p.getPic() != null && !"".equals(p.getPic())) {
          ret = UserInfoFactory.instance().createUserInfo(pic, p.getPic(),
