...
<c:set var="CHILD"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.CHILD%>" />
<c:set var="PARENT"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.PARENT%>" />
<c:set var="MEMBER"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.FAMILY_MEMBER%>" />
<c:set var="DEPENDANT"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.DEPENDANT%>" />
<c:set var="FATHER"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.FATHER%>" />
<c:set var="MOTHER"
	value="<%=fi.koku.services.entity.customerservice.model.CommunityRole.MOTHER%>" />
...
<div class="koku-pyh">
	<div class="portlet-section-body" id="portlet-section-body">
		...
		<div class="pyh-temp">
			<span class="pyh-right"> <a href="${backURL}"> <spring:message
						code="ui.pyh.back" /></a>
			</span>
		</div>
				...
				<table class="portlet-table-body" width="100%">
					...
					<c:forEach var="child" items="${dependants}">
						<tr>
							...
							<td><span class="actions"> <span class="pyh-link">
										<c:choose>

											<c:when test="${child.memberOfUserFamily}">
												<span class="pyh-link"> <portlet:actionURL
					...
					<c:forEach var="familyMember" items="${otherFamilyMembers}">
						<tr>
							...
							<td><span class="pyh-linkki"> <portlet:actionURL
										var="removeFamilyMember">
		...
		<div class="pyh-add-family">
			...
			<h3 class="portlet-section-subheader">
				<spring:message code="ui.pyh.add.family" />
			</h3>
			...
			<form:form name="searchUsersForm" method="post"
				...
				<span class="portlet-form-field-label"><spring:message
						code="ui.pyh.form.last.name" /></span>
				<span class="portlet-form-input-field"> <input
					name="searchSurname" />
				</span>

				<span class="portlet-form-field-label"><spring:message
						code="ui.pyh.table.pic" />: </span>
				<span class="portlet-form-input-field"> <input
					name="searchPic" />
				</span>
				<input class="portlet-form-button" type="submit"
					value="<spring:message code="ui.pyh.seek"/>" />
			</form:form>
		</div>
		...
		<c:choose>
			<c:when test="${not empty searchedUsers}">
				<form:form name="addUsersToFamily" method="post"
					...
					<table class="portlet-table-body" width="100%" border="0">
						...
						<input name="familyCommunityId" type="hidden"
							value="${familyCommunityId}" />

						<c:forEach var="user" items="${searchedUsers}">

							<tr>
								<td><c:out value="${user.firstname} ${user.surname}" /></td>
								<td><c:out value="${user.pic}" /> <input name="userPic"
									type="hidden" value="${user.pic}" /></td>

								<td></td>

								<td><select name="userRole" class="syntmaika">
										<option value="${MEMBER}">
											<spring:message code="ui.pyh.${MEMBER.roleID}" />
										</option>
										<option value="${CHILD}">
											<spring:message code="ui.pyh.${CHILD.roleID}" />
										</option>

										<c:if test="${not parentsFull}">
											<option value="${FATHER}">
												<spring:message code="ui.pyh.${FATHER.roleID}" />
											</option>
											<option value="${MOTHER}">
												<spring:message code="ui.pyh.${MOTHER.roleID}" />
											</option>
											<option value="${PARENT}">
												<spring:message code="ui.pyh.${PARENT.roleID}" />
											</option>
										</c:if>
								</select></td>
							</tr>
					...
					<input class="portlet-form-button" type="submit"
						value="<spring:message code="ui.pyh.save" />" class="tallenna" />
				</form:form>
			...
			<c:otherwise>
				<c:if test="${search}">
					<p>
						<spring:message code="ui.pyh.no.results" />
					</p>
				</c:if>
			</c:otherwise>
		</c:choose>
		...
		<c:if test="${childsGuardianshipInformationNotFound}">
			<p>
				<spring:message code="ui.pyh.cannot.send.request" />
			</p>
		</c:if>
